namespace Gitle.Web.Controllers
{
    ...
    public class IssueController : SecureController
    {
        ...
        public void BookTime(string projectSlug, int issueId, DateTime date, double minutes, bool close, string comment, string status)
        {
            ...
            if (issue.IsArchived) return;
            var booking = new Booking {User = CurrentUser, Date = date, Minutes = minutes, Issue = issue, Project = project, Comment = comment, Unbillable = issue.Unbillable};
        ...
        public object Autocomplete(long projectId, string query)
        {
            ...
            suggestions.AddRange(issues.ToList().Where(i => i.HasBeenOpenSince(DateTime.Today.AddDays(-7))).Select(x => new Suggestion($"#{x.Number} - {x.Name}", x.Id.ToString(), x.Unbillable ? "unbillable" : string.Empty)));
